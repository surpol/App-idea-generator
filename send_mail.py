...
def idea():
	if request.method == 'POST':
		...
		msg = Message(sender='email', recipients=['email', 'email'],body=idea, subject='New Idea Submission')
		ideaEmail = request.form['ideaEmail']
		submission = "Sent by: " + ideaEmail + "\n" + "App Name: " + appName + "\n" + "Idea Description: " + idea
		msg = Message(sender='email', recipients=['email', 'email'],body=submission, subject='New Idea Submission')
		mail.send(msg)
